...
function renderMenu(categories) {
            ...
            card.innerHTML = `
                <div class="relative h-56 overflow-hidden">
                    ...
                    <div class="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                        ...
                        <span class="text-xl font-bold text-primary bg-black/50 px-3 py-1 rounded-full backdrop-blur-sm">\u20AC${(item.price || 0).toFixed(2)}</span>
                    </div>
                </div>
    ...
    if (totalItems === 0) {
        menuContainer.innerHTML = '<div class="col-span-full text-center py-20 text-secondary">No items found.</div>';
    }
}

function applyMenuFilter(query) {
    const allCategories = window.__menuCategories;
    ...
    for (const [catKey, catData] of Object.entries(allCategories)) {
        ...
        if (matchedItems.length > 0) {
            filtered[catKey] = { ...catData, items: matchedItems };
        }
    }
    renderMenu(filtered);
    showFilterBanner(query);
}

function showFilterBanner(query) {
    const existing = document.getElementById('menu-filter-banner');
    ...
    banner.innerHTML = `
        <span class="text-primary font-semibold text-sm">${labels[lang] || labels.en} <span class="text-white text-base">"${query}"</span></span>
